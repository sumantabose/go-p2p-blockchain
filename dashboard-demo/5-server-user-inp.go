...
func handlePOST(w http.ResponseWriter, r *http.Request) {
    ...
    var pvals ProductVals

    decoder := json.NewDecoder(r.Body)
    if err := decoder.Decode(&pvals); err != nil {
        respondWithJSON(w, r, http.StatusBadRequest, r.Body)
        return
    }
    defer r.Body.Close()

    fmt.Println("Received in handlePOST():", pvals)
    newProduct := Product{}
    newProduct.SerialNo = len(ProductData) + 1
    newProduct.Location = 1
    newProduct.CodeNo = pvals.CodeNo
    newProduct.Name = pvals.Name
