...
func makeBasicHost(listenPort int, secio bool, randseed int64) (host.Host, error) {
	...
	if secio {
		log.Printf("Now run \"go run main-noexit.go -l %d -d %s -secio\" on a different terminal\n", listenPort+1, fullAddr)
	} else {
		log.Printf("Now run \"go run main-noexit.go -l %d -d %s\" on a different terminal\n", listenPort+1, fullAddr)
	}
