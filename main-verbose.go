...
func makeBasicHost(listenPort int, secio bool, randseed int64) (host.Host, error) {
	...
	log.Println("basicHost = ", basicHost)
	log.Println("basicHost.ID() = ", basicHost.ID())
	log.Println("basicHost.ID().Pretty() = ", basicHost.ID().Pretty())
	//log.Println("libp2p.Identity(priv).Pretty() = ", libp2p.Identity(priv).Pretty())
...
func main() {
	...
	if *target == "" {
		...
		log.Println("pid = ", pid)
		log.Println("ma.P_IPFS = ", ma.P_IPFS)
		if err != nil {
			log.Fatalln(err)
		}
		...
		log.Println("targetPeerAddr = ", targetPeerAddr)
